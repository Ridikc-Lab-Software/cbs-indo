<body>
    ...
    <a target="blank" href="cetak.php?berdasarkan=data_supir&jenis=xls&pakaiperperiode=<?php echo $status_pakaiperperiode; ?>">
        <?php btn_export("Export Excel"); ?>
    </a>

    <a target="blank" href="cetak.php?berdasarkan=data_supir&jenis=print&pakaiperperiode=<?php echo $status_pakaiperperiode; ?>">
        <?php btn_cetak("Cetak"); ?>
    </a>
        ...
        <table <?php tabel(100, "%", 1, "left"); ?>>
            ...
            <tbody>
                ...
                if (
                    ...
                    $querytabel = "SELECT * FROM data_supir where $berdasarkan like '%$isi%'  LIMIT $startRow ,$dataPerPage";
                    $querypagination = "SELECT COUNT(*) AS total FROM data_supir where $berdasarkan like '%$isi%'";
                } else {
                    $querytabel = "SELECT * FROM data_supir  LIMIT $startRow ,$dataPerPage";
                    $querypagination =
                        "SELECT COUNT(*) AS total FROM data_supir";
                }
                ...
                while ($data = mysql_fetch_array($proses)) { ?>
                    <tr class="event2">
                            ...
                            <table style="border-collapse: collapse; border: none;">
                                <tr>
                                    <td style="padding: 2px; border: none;">
                                        <a href="<?php index(); ?>?input=detail&proses=<?= encrypt($data["id_supir"]) ?>">
                                            <?php btn_detail("Detail"); ?>
                                        </a>
                                    </td>
                                    <td style="padding: 2px; border: none;">
                                        <a href="<?php index(); ?>?input=edit&proses=<?= encrypt($data["id_supir"]) ?>">
                                            <?php btn_edit("Edit"); ?>
                                        </a>
                                    </td>
                                    <td style="padding: 2px; border: none;">
                                        <a href="<?php index(); ?>?input=hapus&proses=<?= encrypt($data["id_supir"]) ?>">
                                            <?php btn_hapus("Hapus"); ?>
                                        </a>
                                    </td>
                                </tr>
                        ...
                        <td align="center" width="50"><?php $no = $no + 1;
                                                        echo $no; ?></td>
                        <!--h <td align="center"><?php echo $data["id_supir"]; ?></td> h-->
