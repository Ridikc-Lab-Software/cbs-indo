<body>

    <form name="formcari" id="formcari" action="" method="get">
        <fieldset>
            <table>
                <tbody>
                    <tr>
                        <td>Berdasarkan</td>
                        <td>:</td>
                        <td>
                            <!-- <input value="" name="Berdasarkan" id="Berdasarkan" > -->
                            <select class="form-control selectpicker" data-live-search="true" name="Berdasarkan"
                                id="Berdasarkan">
                                <?php
                                $sql = "desc data_relasi";
                                $result = @mysql_query($sql);
                                while ($row = @mysql_fetch_array($result)) {
                                    echo "<option name='berdasarkan' value=$row[0]>$row[0]</option>";
                                }
                                ?>
                            </select>
                        </td>
                    </tr>

                    <tr>
                        <td>Pencarian</td>
                        <td>:</td>
                        <td>
                            <!--<input class="form-control" type="text" name="isi" value="" >--> <input type="text"
                                name="isi" value="">
                            <?php btn_cari('Cari'); ?>
                        </td>
                    </tr>
                </tbody>
            </table>
        </fieldset>
    </form>

    <div class="table-responsive mt-4">
        <table <?php tabel(100, '%', 1, 'left'); ?>>
            <tr <?php tabel_head_tr() ?>>
                <th>Action</th>
                <th>No</th>
                <!--h <th>Id Relasi </th> h-->
                <th align="center" class="th_border cell">Nama </th>
                <th align="center" class="th_border cell">Nomor Telepon </th>
                <th align="center" class="th_border cell">Email </th>
                <th align="center" class="th_border cell">Alamat </th>
                <th align="center" class="th_border cell">Nama Spbu </th>
                <th align="center" class="th_border cell">Password </th>
                <th align="center" class="th_border cell">Hapus </th>

            </tr>

            <tbody>
                <?php
                $no = 0;
                $startRow = ($page - 1) * $dataPerPage;
                $no = $startRow;

                if (isset($_GET['Berdasarkan']) && !empty($_GET['Berdasarkan']) && isset($_GET['isi']) && !empty($_GET['isi'])) {
                    $berdasarkan = mysql_real_escape_string($_GET['Berdasarkan']);
                    $isi = mysql_real_escape_string($_GET['isi']);
                    $querytabel = "SELECT * FROM data_relasi where $berdasarkan like '%$isi%'  LIMIT $startRow ,$dataPerPage";
                    $querypagination = "SELECT COUNT(*) AS total FROM data_relasi where $berdasarkan like '%$isi%'";
                } else {
                    $querytabel = "SELECT * FROM data_relasi  LIMIT $startRow ,$dataPerPage";
                    $querypagination = "SELECT COUNT(*) AS total FROM data_relasi";
                }
                $proses = mysql_query($querytabel);
                while ($data = mysql_fetch_array($proses)) {
                ?>
                    <tr class="event2">


                    <td class="th_border cell" align="center" >
                            
                             <a style="
    margin-left: 11px;
" href="../data_voucher/?input=list_detail&id=<?php echo encrypt($data['id_relasi']); ?>" class="btn btn-block btn-sm btn-light-success font-weight-bolder text-uppercase py-4">
                             
                             Detail&nbsp;Relasi & E-voucher</a>
                                   
                        </td>


                       

                        <td align="center" width="50"><?php $no = (($no + 1));
                                                        echo $no; ?></td>
                        <!--h <td align="center"><?php echo $data['id_relasi']; ?></td> h-->
                        <td align="left"><b><?php echo $data['nama']; ?></b></td>
                        <td align="center"><?php echo $data['nomor_telepon']; ?></td>
                        <td align="center"><?php echo $data['email']; ?></td>
                        <td align="center"><?php echo $data['alamat']; ?></td>
                        <td align="center">
                            <?php echo baca_database("", "nama_spbu", "select * from data_spbu where id_spbu='$data[id_spbu]'") ?>
                        </td>
                        <td align="center"><?php echo $data['password']; ?></td>
 <td class="th_border cell" align="center" width="200">
                            
                                        <a href="<?php index(); ?>?input=hapus&proses=<?= encrypt($data['id_relasi']); ?>">
                                            <?php btn_hapus('Hapus'); ?>
                                        </a>
                            
                        </td>

                    </tr>
                <?php } ?>
            </tbody>
        </table>
    </div>

    <?php Pagination($page, $dataPerPage, $querypagination); ?>

</body>