...
<table border="0" style="width: 100%">
    ...
    <tr>
        <td colspan="13" align="center" class="auto-style1">
            <center>
                <strong>
    ...
    <tr>
        <td colspan="13" align="center" class="auto-style2">
            <center>
                <h2 style="margin: 0px;" class="auto-style1">LAPORAN TRANSAKSI E-VOUCHER</h2>
            </center>
        </td>
    </tr>
    ...
    <tr>
        <td colspan="13" align="center" class="auto-style2">
            <center>SPBU
                <?php $id_admin = decrypt($_COOKIE['kodene']);
    ...
    <tr>
        <td class="auto-style2">
            <center>
                <h2 class="auto-style1" style="margin: 0px;">LAPORAN TRANSAKSI E-VOUCHER</h2>
            </center>
        </td>
    </tr>
    <tr>
...
<?php
    ...
    if (isset($_GET['tanggal1']) && !empty($_GET['tanggal1'])) {
        ...
        $shift_text = "";
        if ($filter_shift != '') {
            $shift_name = baca_database("", "shift", "SELECT shift FROM data_shift WHERE id_shift='$filter_shift'");
            $shift_text = " | Shift: <b>" . $shift_name . " (" . $jam_mulai . " - " . $jam_selesai . ")</b>";
        }

        $text_cetak = 'Cetak Berdasarkan <b>tanggal_transaksi</b> Dari Tanggal <b>' . $tanggal1_indo . '</b> s/d <b>' . $tanggal2_indo . '</b>' . $shift_text;
    }

    if ($text_cetak != "") {
        if (isset($_GET['export'])) {
            echo '<table width="100%" border="0"><tr><td colspan="13" align="center"><center>' . $text_cetak . '</center></td></tr></table><br>';
        }
        else {
...
<!-- BODY -->
<table width="100%" <?php echo isset($_GET['export']) ? 'border="1" style="border-collapse:collapse;"'
        : 'class="tblcms2"'; ?>>
    <tr>
        ...
        <th align="center" class="th_border cell">Nama Member</th>
        ...
        <th align="center" class="th_border cell">Transaksi Voucher</th>
        <th align="center" class="th_border cell">Sisa Voucher</th>
    ...
    $path = "https://membercard.cbs-indo.com/admin/upload/";
    ...
    while ($data = mysql_fetch_array($proses)) {
        ...
        $supir_data_row = mysql_fetch_assoc(mysql_query("SELECT ds.nama_supir, ds.id_member FROM data_supir ds 
        ...
        $supir = ($supir_data_row && $supir_data_row['nama_supir'] !== '') ? $supir_data_row['nama_supir'] : '-';
        $supir_id_member = ($supir_data_row && !empty($supir_data_row['id_member'])) ? $supir_data_row['id_member'] : '';
        $nama_member_supir = '';
        if ($supir_id_member !== '') {
            $nama_member_supir = baca_database("", "nama", "SELECT nama FROM data_member WHERE id_member = '$supir_id_member' LIMIT 1");
        }
        $nama_member_supir = !empty($nama_member_supir) ? $nama_member_supir : 'Non Member';

        $foto = baca_database("", "foto", "SELECT foto FROM data_plat_kendaraan_transaksi_voucher 
                                                   WHERE id_transaksi_voucher = '$id_transaksi_voucher'");


        $nominal_voucher = baca_database("", "nominal_voucher", "SELECT nominal_voucher FROM data_sisa_voucher WHERE id_voucher='{$data['id_voucher']}'");
        $sisa_voucher = baca_database("", "nominal_sisa", "SELECT nominal_sisa FROM data_sisa_voucher WHERE id_voucher='{$data['id_voucher']}'");
        $total_sisa = $total_sisa + $sisa_voucher;
...
?>
        <tr class="event2">
            ...
            <td>
                <?php echo baca_database("", "nama", "SELECT nama FROM data_relasi WHERE id_relasi='{$data['id_relasi']}'"); ?>
            </td>
            <td align="center">
                <?php echo format_indo_jam($data['tanggal_transaksi']); ?>
            </td>
            <td align="center">
                <?php echo get_shift($data['tanggal_transaksi'], $shifts); ?>
            </td>
            <td align="center">ID
                <?php echo baca_database("", "id_voucher", "SELECT id_voucher FROM data_voucher WHERE id_voucher='{$data['id_voucher']}'"); ?>
            </td>
            <td>
                <?php echo $supir; ?>
            </td>
            <td>
                <?php echo $nama_member_supir; ?>
            </td>
            <td>
                <?php echo $plat; ?>
            </td>
        ...
        if (!empty($foto)) {
            ...
            foreach ($list_foto as $img) {
                ...
                if ($img != "") { ?>
                <?php if (isset($_GET['export'])) { ?>
                <img src="<?php echo $path . $img; ?>" width="20" height="20"
...
?>
        ...
        switch ($jenis_bbm) {
            case "TURBO":
                $TURBO++;
                $JML_TURBO += $data['nominal'];
                break;
            case "DEXLITE":
                $DEXLITE++;
                $JML_DEXLITE += $data['nominal'];
                break;
            case "PERTAMAX":
                $PERTAMAX++;
                $JML_PERTAMAX += $data['nominal'];
                break;
            case "PERTALITE":
                $PERTALITE++;
                $JML_PERTALITE += $data['nominal'];
                break;
        }
?>
            </td>
            <td class="text-right">
                <?php echo rupiah($nominal_voucher); ?>
            </td>
            <td class="text-right">
                <?php echo rupiah($data['nominal']); ?>
            </td>
            <td class="text-right">
                <?php echo rupiah($sisa_voucher); ?>
            </td>
            <td align="center">
                <?php
        ...
        switch ($jenis_bbm) {
            case "TURBO":
                $JML_TURBO_LITER += $liter;
                break;
            case "DEXLITE":
                $JML_DEXLITE_LITER += $liter;
                break;
            case "PERTAMAX":
                $JML_PERTAMAX_LITER += $liter;
                break;
            case "PERTALITE":
                $JML_PERTALITE_LITER += $liter;
                break;
        }
?>
            </td>
        ...
        <tr>
            <td colspan="10" class="text-right"><strong>Jumlah Voucher :
                    <?php echo $no; ?>
                </strong></td>
            ...
            <td class="text-right"><strong>
                    <?php echo rupiah($total_sisa); ?>
                </strong></td>
            <td align="center"><strong>
                    <?php echo number_format($total_liter, 2, ',', '.'); ?>
                </strong></td>
        </tr>
    </tbody>
</table>
...
<table border="0" style="border: 0px solid #ddd; width: 100%">
    <tr>
        ...
        <td valign="top">
            ...
            <br>
            <table <?php echo isset($_GET['export']) ? 'border="1" style="border-collapse:collapse; width: 100%;"'
            : 'style="width: 60%;"'; ?>>
                <tr>
                    <th colspan="4" align="left">Summary Jumlah Transaksi per Jenis BBM dan Total Liter</th>
                </tr>
                <tr>
                    <th align="center">Product</th>
                    <th align="center">Jumlah Voucher</th>
                    <th align="center">Nominal</th>
                    <th align="center">Total Liter</th>
                </tr>
                <tr>
                    <td>TURBO</td>
                    <td align="center">
                    ...
                    <td align="right">
                        <?php echo rupiah($JML_TURBO); ?>
                    </td>
                    <td align="right">
                        <?php echo number_format($JML_TURBO_LITER, 2, ',', '.'); ?> L
                    </td>
                </tr>
                <tr>
                    <td>DEXLITE</td>
                    ...
                    <td align="right">
                        <?php echo rupiah($JML_DEXLITE); ?>
                    </td>
                    <td align="right">
                        <?php echo number_format($JML_DEXLITE_LITER, 2, ',', '.'); ?> L
                    </td>
                </tr>
                <tr>
                    <td>PERTAMAX</td>
                    ...
                    <td align="right">
                        <?php echo rupiah($JML_PERTAMAX); ?>
                    </td>
                    <td align="right">
                        <?php echo number_format($JML_PERTAMAX_LITER, 2, ',', '.'); ?> L
                    </td>
                </tr>
                <tr>
                    <td>PERTALITE</td>
                    ...
                    <td align="right">
                        <?php echo rupiah($JML_PERTALITE); ?>
                    </td>
                    <td align="right">
                        <?php echo number_format($JML_PERTALITE_LITER, 2, ',', '.'); ?> L
                    </td>
                </tr>
            </table>
        </td>
