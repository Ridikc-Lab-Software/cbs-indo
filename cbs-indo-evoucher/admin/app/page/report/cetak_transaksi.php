    ...
    <td>
        <select class="form-control selectpicker mt-2" data-live-search="true" name="nominal">
            ...
            while ($d = mysql_fetch_array($q)) {
                echo '<option value="'.$d['nominal'].'">'.r_rupiah($d['nominal']).'</option>';
            }
            ?>
        </select>
    </td>
    ...
    <td>
        <select class="form-control selectpicker mt-2" data-live-search="true" name="shift">
            ...
            while ($r_shift = mysql_fetch_array($q_shift)) {
                echo '<option value="'.$r_shift['id_shift'].'">Shift '.$r_shift['shift'].' ('.$r_shift['jam_mulai'].' - '.$r_shift['jam_selesai'].')</option>';
            }
            ?>
        </select>
    </td>
