...
{
	// 1. Ambil page & perPage dengan validasi
	$page        = (isset($_GET['page']) && is_numeric($_GET['page']) && $_GET['page'] >= 1) ? (int)$_GET['page'] : 1;
	$dataPerPage = (isset($_GET['perPage']) && is_numeric($_GET['perPage']) && $_GET['perPage'] >= 1) ? (int)$_GET['perPage'] : 10;

	// 2. Bangun URL dasar yang sudah membawa SEMUA parameter GET kecuali 'page'
	$params = $_GET;
	unset($params['page']);                    // kita tambahkan manual nanti
	$base_url = $_SERVER['PHP_SELF'];           // misal: /index.php
	$query_string = http_build_query($params);  // jadi string query (tanpa page)
	$page_url = $base_url . ($query_string ? '?' . $query_string . '&' : '?');

	// 3. Hitung total data
	$countTotalRow = mysql_query($querypagination) or die(mysql_error());
	$row           = mysql_fetch_assoc($countTotalRow);
	$totalRow      = $row['total'];

	$total_pages = $totalRow > 0 ? ceil($totalRow / $dataPerPage) : 1;
	if ($page > $total_pages) $page = $total_pages;

	// 4. Tampilkan info jumlah data & halaman
	echo "Jumlah {$totalRow} data, ";
	echo "Halaman {$page} dari {$total_pages} Halaman<br><br>";

	// 5. Mulai buat link pagination
	$pagination = '';

	// First + Previous
	$pagination .= '<a class="btn btn-info btn-xs" href="' . $page_url . 'page=1" title="First">First</a> ';
	$prev = ($page <= 1) ? 1 : $page - 1;
	$pagination .= '<a class="btn btn-info btn-xs" href="' . $page_url . 'page=' . $prev . '">Previous Sebelumnya</a> ';

	// Nomor halaman (tampilkan sekitar halaman aktif)
	$start = max(1, $page - 2);
	$end   = min($total_pages, $page + 3);

	for ($i = $start; $i <= $end; $i++) {
		if ($i == $page) {
			$pagination .= '<a class="btn btn-default btn-xs">' . $i . '</a> ';
		} else {
			$pagination .= '<a class="btn btn-info btn-xs" href="' . $page_url . 'page=' . $i . '">' . $i . '</a> ';
		}
	}

	// Next + Last
	$next = ($page >= $total_pages) ? $total_pages : $page + 1;
	$pagination .= '<a class="btn btn-info btn-xs" href="' . $page_url . 'page=' . $next . '">Berikutnya Next</a> ';
	$pagination .= '<a class="btn btn-info btn-xs" href="' . $page_url . 'page=' . $total_pages . '" title="Last">Last</a> ';
