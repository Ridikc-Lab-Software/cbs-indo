package com.project.aplikasi.petugas_cbs.combobox_data_petugas;

import com.project.aplikasi.petugas_cbs.config.config_apiclient;
import static com.project.aplikasi.petugas_cbs.config.config_global.BASE_URL;

public class combobox_data_petugas_apiutils {
    public static combobox_data_petugas_apiservice getAPIService() {
        return config_apiclient.getClient(BASE_URL).create(combobox_data_petugas_apiservice.class);
    }
}




