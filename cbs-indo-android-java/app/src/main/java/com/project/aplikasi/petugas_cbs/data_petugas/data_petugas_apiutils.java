package com.project.aplikasi.petugas_cbs.data_petugas;

import com.project.aplikasi.petugas_cbs.config.config_apiclient;

import static com.project.aplikasi.petugas_cbs.config.config_global.BASE_URL;

public class data_petugas_apiutils {


    public static data_petugas_apiservice getAPIService() {
        return config_apiclient.getClient(BASE_URL).create( data_petugas_apiservice.class);
    }
}


