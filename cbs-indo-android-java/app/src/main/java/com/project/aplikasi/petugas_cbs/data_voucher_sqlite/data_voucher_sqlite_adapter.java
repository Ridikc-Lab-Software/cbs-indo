...
public class data_voucher_sqlite_adapter extends RecyclerView.Adapter<data_voucher_sqlite_adapter.data_voucher_ViewHolder> {

    Activity activity;
    private List<data_voucher_sqlite_data> datalist = new ArrayList<>();
    private data_voucher_sqlite_dbhandler dbHandler;
    private data_voucher_sqlite_adapter adapter;


    public data_voucher_sqlite_adapter(Activity activity,List<data_voucher_sqlite_data> datalist) {
        this.activity = activity;
        this.datalist = datalist;
    }

    @Override
    public data_voucher_ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.data_voucher_tampil_v2, parent, false);
        data_voucher_ViewHolder mahasiswaViewHolder = new data_voucher_ViewHolder(view);
        return mahasiswaViewHolder;
    }

    @Override
    public void onBindViewHolder(data_voucher_ViewHolder holder, int position) {
        holder.txt_result_id_voucher.setText("id_voucher : " + datalist.get(position).get_id_voucher());
       holder.txt_result_qrcode.setText("Qrcode  : " + Html.fromHtml(datalist.get(position).get_qrcode()+ "" ));
       ...
       holder.txt_result_tanggal_kadaluarsa.setText("Tanggal Kadaluarsa  : " + Html.fromHtml(datalist.get(position).get_tanggal_kadaluarsa()+ "" ));
       holder.txt_result_id_spbu.setText("Id Spbu  : " + Html.fromHtml(datalist.get(position).get_id_spbu()+ "" ));
       holder.txt_result_id_penjualan_voucher.setText("Id Penjualan Voucher  : " + Html.fromHtml(datalist.get(position).get_id_penjualan_voucher()+ "" ));
       holder.txt_result_status.setText("Status  : " + Html.fromHtml(datalist.get(position).get_status()+ "" ));
       holder.txt_result_file_voucher.setText("File Voucher  : " + Html.fromHtml(datalist.get(position).get_file_voucher()+ "" ));
    ...
    public class data_voucher_ViewHolder extends RecyclerView.ViewHolder {
        TextView txt_result_id_voucher;
       TextView txt_result_qrcode;
       TextView txt_result_id_relasi;
       TextView txt_result_nominal;
       TextView txt_result_tanggal_kadaluarsa;
       TextView txt_result_id_spbu;
       TextView txt_result_id_penjualan_voucher;
       TextView txt_result_status;
       TextView txt_result_file_voucher;
       TextView txt_result_tanggal_dibuka;

        LinearLayout linearEdit, linearHapus;
        protected int REQUEST_CODE_TAMBAH = 3543;
        public data_voucher_ViewHolder(final View itemView) {
            super(itemView);
            dbHandler = new data_voucher_sqlite_dbhandler(activity);
            txt_result_id_voucher = (TextView) itemView.findViewById(R.id.id_voucher);
           txt_result_qrcode = (TextView) itemView.findViewById(R.id.qrcode);
           ...
           txt_result_tanggal_kadaluarsa = (TextView) itemView.findViewById(R.id.tanggal_kadaluarsa);
           ...
           txt_result_file_voucher = (TextView) itemView.findViewById(R.id.file_voucher);
            ...
            linearEdit.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                   ...
                   bundle.putString("tanggal_dibuka", datalist.get(getAdapterPosition()).get_tanggal_dibuka());
                    ...
                    Intent intent = new Intent(activity, data_voucher_sqlite_edit.class);
                    ...
                    BackAlertDialog.setPositiveButton("Ya",
                            new DialogInterface.OnClickListener() {
                                public void onClick(DialogInterface dialog, int which) {
                                    ...
                                    dbHandler.hapus_data_voucher_sqlite( new data_voucher_sqlite_data(
                                            datalist.get(getAdapterPosition()).get_id_voucher()
                                           ,datalist.get(getAdapterPosition()).get_qrcode()
                                           ,datalist.get(getAdapterPosition()).get_id_relasi()
                                           ,datalist.get(getAdapterPosition()).get_nominal()
                                           ,datalist.get(getAdapterPosition()).get_tanggal_kadaluarsa()
                                           ...
                                           ,datalist.get(getAdapterPosition()).get_file_voucher()
                                    ...
                                    adapter = new data_voucher_sqlite_adapter( activity, data_voucher_sqliteList );
                                    adapter.notifyDataSetChanged();
                                    Toast.makeText( activity, "Berhasil Terhapus", Toast.LENGTH_SHORT ).show();
                                    ((data_voucher_sqlite_activity)activity).fetch_data_voucher_sqlite();
                                }
                            });
